...
conn, addr = s.accept()
while 1:
    length1 = recv_size(conn,16) #首先接收来自客户端发送的大小信息
    if isinstance (length1,str): #若成功接收到大小信息，进一步再接收整张图片
        side1 = recv_size(conn,16) #首先接收来自客户端发送的大小信息
        stringData1 = recv_all(conn, int(length1))
        length2 = recv_size(conn,16)
        side2 = recv_size(conn,16)
